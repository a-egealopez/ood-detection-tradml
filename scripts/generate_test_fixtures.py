...
HOUSE_PROFILES = {
    "aruba": dict(events_mean=180, night_ratio=0.08, n_days=60, seed=1),
    "cairo": dict(events_mean=90, night_ratio=0.05, n_days=45, seed=2),
    "milan": dict(events_mean=260, night_ratio=0.15, n_days=50, seed=3),
    "tulum": dict(events_mean=130, night_ratio=0.10, n_days=40, seed=4),
}
...
def _events_for_day(rng, date, events_mean, night_ratio, anomalous=False):
    ...
    hours = np.where(
        is_night,
        rng.choice(list(range(22, 24)) + list(range(0, 8)), size=n_events),
    ...
    timestamps = [
        pd.Timestamp(date) + pd.Timedelta(hours=int(h), minutes=int(m), seconds=int(s), microseconds=int(u))
    ...
    for ts in timestamps:
        ...
        rows.append((ts.strftime("%Y-%m-%d"), ts.strftime("%H:%M:%S.%f"), sensor, reading))

    return rows


def generate_house_csv(house_id: str, events_mean: int, night_ratio: float, n_days: int, seed: int) -> Path:
    ...
    for day_offset in range(n_days):
        ...
        all_rows.extend(
            _events_for_day(rng, date, events_mean, night_ratio, anomalous=day_offset in anomalous_days)
        )
...
if __name__ == "__main__":
    for house_id, profile in HOUSE_PROFILES.items():
        ...
        print(f"✓ [{house_id}] {path} ({profile['n_days']} días, ~{profile['events_mean']} eventos/día)")