...
- Python ≥3.10, Streamlit UI, Plotly charts (always use Plotly in Streamlit), scikit-learn,
  numpy, pandas, scipy, `hmmlearn` (sequential), `pyod` (LOF).
...
│   ├── detectors/
│   │   ├── __init__.py         # Public API: 13 detectors + EnsembleDetector
...
│   ├── fetch_casas_data.py     # Download + convert real CASAS (Zenodo CC-BY-4.0) into data/real/
...
- The collective gate in `verify_pipeline.py` is **source-aware**: it is a hard gate only
  ...
  self-loops excluded) is ~0.35 on the synthetic houses and **0.68–0.83 on the real CASAS
  data** (aruba 0.68, cairo 0.83, milan 0.73, tulum 0.80 — all below the 0.85 threshold,
  so the reversal injector does produce rare transitions on these real streams). On
  near-symmetric data the collective gates are reported as informational instead of
...
- **HawkesDetector** uses a real intensity-based score (negative conditional
  ...
  numpy, no `tick` dependency). The Hawkes Poisson
  intensity is valid only for counts,
...
| Task | Command |
|---|---|
| Launch the app | `scripts/run.sh` (Linux/WSL) or `scripts/run.bat` (Windows) — auto-detects source: uses `data/real/*.csv` if present, otherwise falls back to synthetic fixtures |
...
| Fetch real CASAS data | `python scripts/fetch_casas_data.py` (Zenodo CC-BY-4.0 -> `data/real/`) |
...
(bootstraps venv + deps) or `pip install -r requirements.txt` inside the venv.

**Real CASAS data** comes from the WSU CASAS smart-home datasets (houses `aruba`,
`cairo`, `milan`, `tulum`), published on Zenodo under CC-BY-4.0 (Cook, 2025, DOI:
`10.5281/zenodo.17180309`). `data/real/` is gitignored: `scripts/fetch_casas_data.py`
downloads `new_labeled_data.zip`, filters motion/door events (`M###`/`D###`) and drops
activity labels and temperature readings, writing `data/real/casas_<house>_raw.csv` in
the loader's schema (see `raw_csv_path` in `src/config.py`). `scripts/run.sh` and
`scripts/run.bat` pick them up automatically; `--real` forces them and `--synthetic`
forces the synthetic fixtures.
